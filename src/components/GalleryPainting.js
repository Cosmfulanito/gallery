...
function GalleryPainting(props) {
	return (
		<Link to={`gallery/${props.subdirectory(props.painting.name)}`} className={`link ${useTextTransform(props.painting.name)}`}>								{/* Haciendo click aquí me permite redirigirme al link de esta imagen en especifico. Cada imagen va a tener un link propio */}
